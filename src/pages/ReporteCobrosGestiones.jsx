...
import { Get_Report_Gestiones_Cobro } from '../services/Service_Api_Reportes'
import { ToastContainer, toast } from 'react-toastify';
const ReporteCobrosGestiones = () => {
  ...
  const descargarReporteCobros = async (e) => {
    ...
    const functionThatReturnPromise = async () => {
      try {
        result = await Get_Report_Gestiones_Cobro(dateInit.format('YYYY-MM-DD'), dateEnd.add(1, 'day').format('YYYY-MM-DD'));
      } catch (error) {
        throw error;
      }
    };
...
export default ReporteCobrosGestiones