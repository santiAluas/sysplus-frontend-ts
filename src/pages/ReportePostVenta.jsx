...
import { GET_REPORTE_POST_VENTA } from '../services/Service_Post_Venta'
import { ToastContainer, toast } from 'react-toastify';
const ReportePostVenta = () => {
  ...
  const descargarReporteCobros = async (e) => {
    ...
    const functionThatReturnPromise = async () => {
      try {
        result = await GET_REPORTE_POST_VENTA(dateInit.format('YYYY-MM-DD'), dateEnd.add(1, 'day').format('YYYY-MM-DD'));
      } catch (error) {
        throw error;
      }
    };
  ...
  return (
    <Box component="fieldset" pt={2} pb={4} pl={2} pr={2}>
      <legend>REPORTE POST VENTA</legend>
      <ToastContainer />
...
export default ReportePostVenta