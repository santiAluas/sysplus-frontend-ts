...
import { UPLOAD_LIST_PRODUCT_INVENTARY } from '../services/Api_Inventario/Api_TomaFisicaInventario.js'
import { ToastContainer, toast } from 'react-toastify';
import excel_image from '../assets/images/excel_image.png'

const UploadProductListExcel = () => {
    ...
    function createData(...args) {
        ...
        args.forEach((value, index) => {
            ...
            let valor = value === "" || value === undefined || value === 'undefined' ?"0":value.toString();
            if (typeof valor === 'string') {
                valor = valor.replace(/'/g, '');
            }
           concat += `${index > 0 ? ',' : ''} '${valor}'`;
        });

        concat += `,'${userLogin.User}','[fechaactual]','activo', '[nameListPrice]', '[idtomafisica]','${uuidv4()}')`;
        return concat;
    }
    ...
    const InsertExcelListPrice = () => {
        ...
        const functionThatReturnPromise = async () => {
            try {
                await UPLOAD_LIST_PRODUCT_INVENTARY(DataFacturas, nameListPrice);
            } catch (error) {
                throw error;
            }
        };
    ...
    return (
        <>
            ...
            <ToastContainer />
            <NavbarMasterMoto titulo="LISTA DE PRODUCTOS INVENTARIO" />
            <br />
            <Paper elevation={3}
                style={{
                    marginRight: 20,
                            ...
                            <div style={{ marginLeft: '19px', marginTop: '20px' }}>
                                ...
                                {DataFacturas.length > 0 && (
                                    <div className="excel-data-container">
                                        ...
                                        />
                                        <Button variant="contained" onClick={InsertExcelListPrice} fullWidth>SUBIR LISTA PRODUCTOS INVENTARIO</Button>
                                    </div>
                                )}
                            </div>
                        </div>
...
export default UploadProductListExcel