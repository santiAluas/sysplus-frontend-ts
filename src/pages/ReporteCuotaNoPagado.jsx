...
import { GET_REPORTE_CUOTA_CERO_NO_PAGADO } from '../services/Service_Api_Reportes'
import { ToastContainer, toast } from 'react-toastify';
const ReporteCuotaNoPagado = () => {
...
const [dateEnd, setDateEnd] = React.useState(dayjs());
  ...
  const descargarReporteCobros = async (e) => {
    ...
    const functionThatReturnPromise = async () => {
      try {
        result = await GET_REPORTE_CUOTA_CERO_NO_PAGADO(dateInit.format('DD-MM-YYYY'), dateEnd.add(1, 'day').format('DD-MM-YYYY'));
      } catch (error) {
        throw error;
      }
    };
  ...
  return (
    ...
    <legend>CUOTAS CERO NO PAGADO</legend>
    <ToastContainer />
    <Stack spacing={{ xs: 1, sm: 2 }} justifyContent="center" alignItems="center">
      ...
      <Button variant="contained" onClick={(e) => descargarReporteCobros(e)} fullWidth>DESCARGAR EXCEL</Button>
    </Stack>
  </Box>
  )
}

export default ReporteCuotaNoPagado