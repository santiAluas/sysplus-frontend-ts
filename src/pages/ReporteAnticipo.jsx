...
import { DESCARGAR_REPORTE_ANTICIPO } from '../services/AnticiposMatricula'
import { ToastContainer, toast } from 'react-toastify';
const ReporteAnticipo = () => {
  ...
  const descargarReporteCobros = async (e) => {
    ...
    const functionThatReturnPromise = async () => {
      try {
        await DESCARGAR_REPORTE_ANTICIPO(dateInit.format('YYYY-MM-DD'), dateEnd.add(1, 'day').format('YYYY-MM-DD'));
      } catch (error) {
        throw error;
      }
    };
  ...
  return (
    <Box component="fieldset" pt={2} pb={4} pl={2} pr={2}>
      <legend>REPORTE ANTICIPO MATRICULA</legend>
      <ToastContainer />
