...
import { REPORT_PRODUCT_COST } from '../../services/Api_BodegaConsignacion/Api_BodegaConsignacion'

import { ToastContainer, toast } from 'react-toastify';
const ReporteProductoCostos = () => {
  ...
  const descargarReporteCobros = async (e) => {
    ...
    const functionThatReturnPromise = async () => {
      try {
        result = await REPORT_PRODUCT_COST(dateInit.format('YYYY-MM-DD'), dateEnd.add(1, 'day').format('YYYY-MM-DD'));
      } catch (error) {
        throw error;
      }
    };
  ...
  return (
    <Box component="fieldset" pt={2} pb={4} pl={2} pr={2}>
      <legend>REPORTE COSTOS PRODUCTOS</legend>
      <ToastContainer />
      <Stack spacing={{ xs: 1, sm: 2 }} justifyContent="center" alignItems="center">
        ...
        <Stack
          ...
          spacing={2}
          style={{ width: '100%' }}>
          <Button variant="contained" onClick={(e) => descargarReporteCobros(e)} fullWidth>DESCARGAR REPORTE COSTOS</Button>
        </Stack>
      </Stack>
    </Box>
  )
...
export default ReporteProductoCostos